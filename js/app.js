...
function initRegionSelect(){ REGION_LIST.forEach(region=>{ const option=document.createElement("option"); option.value=JSON.stringify({name:region.name,lat:region.lat,lon:region.lon,key:region.key,source:"local"}); option.textContent=region.name; regionResult.appendChild(option); }); const d=REGION_MAP[DEFAULT_REGION_KEY]; currentLocation={name:d.name,lat:d.lat,lon:d.lon,key:d.key,source:"local"}; regionSearch.value=d.name; setRegionResultSingle(currentLocation); renderLocationInfo(currentLocation); }
